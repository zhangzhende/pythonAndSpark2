...
def extractFeatures(field, categoriesMap, featureEnd):
    ...
    categoryFeatures = np.zeros(len(categoriesMap),dtype=float)  # 创建categoriesMap的同型0矩阵
...
def trainEvaluateModel(trainData, validationData, numIterations, stepSize, miniBatchFraction):
    starttime = time()
    # 方法过时，记得换新方法
    model = LogisticRegressionWithSGD.train(data=trainData, iterations=numIterations, step=stepSize,
                                            miniBatchFraction=miniBatchFraction)
    AUC = evaluateModel(model, validationData)
    duration = time() - starttime
    print("训练评估使用参数：\n", "numIterations=", numIterations, "\n stepSize=", stepSize, "\n miniBatchFraction=",
          miniBatchFraction, "====>用时=", duration, "\n 结果AUC=", AUC)
...
def parameterEval(trainData, validationData):
    ...
    evalParammeter(trainData=trainData,
                   ...
                   miniBatchFractionList=[1])
    print("评估miniBatchFraction参数===========")
    evalParammeter(trainData=trainData,
                   validationData=validationData,
                   evalParam="miniBatchFraction",
                   numIterationList=[50],
                   stepSizeList=[500],
...
# 评估摸一个参数
def evalParammeter(trainData, validationData, evalParam, numIterationList, stepSizeList, miniBatchFractionList):
    # 训练评估参数
    metrics = [trainEvaluateModel(trainData, validationData, numIteration, stepSize, miniBatchFraction)
    ...
    elif evalParam == "miniBatchFraction":
        indexList = miniBatchFractionList
    df = pd.DataFrame(data=metrics, index=indexList,
                      columns=["AUC", "duration", "numIteration", "stepSize", "miniBatchFraction", "model"])
...
def evaluateModel(model, validationData):
    ...
    AUCB = metrics.areaUnderROC
    return (AUCB)


# 评估所有参数，选择最佳参数组合
def evalAllParammeter(trainData, validationData, numIterationList, stepSizeList, miniBatchFractionList):
    # for循环遍历所有参数集合
    ...
    # 显示调校后的最佳参数组合
    print("调教的最佳参数: numIterations=", bestParammeter[2], ",stepSize=", bestParammeter[3], ",miniBatchFraction=",
          bestParammeter[4], "结果AUC=", bestParammeter[0])
    # 返回最佳模型
...
if __name__ == "__main__":
    ...
    testData.persist()
    print("训练评估阶段===================")
    (AUC, duration, numIterations, stepSize, miniBatchFraction, model) = trainEvaluateModel(trainData, validationData,
                                                                                            15,10,0.5)
    print("找出最好的参数组合=============")
