...
# 数据转换，将文件的问号转换为0
def converFloat(x):
    ret=(float(0.0) if x == "?" else float(x))
    return (float(0.0) if ret <0.0 else ret)
...
def trainEvaluateModel(trainData,validationData,lambdaParam):
    ...
    """
    starttime=time()
    model=NaiveBayes.train(data=trainData,lambda_=lambdaParam)
    AUC=evaluateModel(model,validationData)
    duration=time()-starttime
    print("训练评估使用参数：\n","lambdaParam=",lambdaParam,"====>用时=",duration,"\n 结果AUC=",AUC)
    return (AUC,duration,lambdaParam,model)

def parameterEval(trainData,validationData):
    print("评估lambdaParam参数===========")
    evalParammeter(trainData=trainData,
                   validationData=validationData,
                   evalParam="numIeteration",
                   lambdaParamList=[1.0,3.0,5.0,15.0,25.0,30.0,40.0,45.0,60.0])



#评估摸一个参数
def evalParammeter(trainData,validationData,evalParam,lambdaParamList):
    # 训练评估参数
...
def evaluateModel(model ,validationData):
    ...
    return (AUC)
#评估所有参数，选择最佳参数组合
def evalAllParammeter(trainData,validationData,lambdaParamList):
    #for循环遍历所有参数集合
    metrics = [trainEvaluateModel(trainData, validationData, lambdaParam)
                for lambdaParam in lambdaParamList]
    #找出AUC最大的参数组合
    ...
    #显示调校后的最佳参数组合
    print("调教的最佳参数: lambdaParam=",bestParammeter[2],"结果AUC=",bestParammeter[0])
    #返回最佳模型
    return bestParammeter[3]
#预测
...
if __name__=="__main__":
    ...
    testData.persist()
    print("训练评估阶段===================")
    (AUC,duration,lambdaParam,model)=trainEvaluateModel(trainData,validationData,6.0)
    print("找出最好的参数组合=============")
